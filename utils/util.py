...
def logger(exp_path, exp_name, work_dir, exception, resume=False):
    ...
    cfg_file = open(r'D:\conda3\Transfer_Learning\CrowdCounting\C3-Framework\C-3-Framework-python3.x\train_SKT\config.py',"r")
...
def print_summary(exp_name,scores,train_record):
    ...
    print( '    '+ '-'*20 )
    print( '[best] [models: %s] , [mae %.2f], [mse %.2f]' % (train_record['best_model_name'],\
...
def update_model(
        net,optimizer,scheduler,epoch,i_tb,exp_path,
        exp_name,scores,train_record,log_file=None
):
    ...
    if mae < train_record['best_mae'] or mse < train_record['best_mse']:
        snapshot_name = 'all_ep_%d_mae_%.1f_mse_%.1f' % (epoch + 1, mae, mse)
        save_path = os.path.join(exp_path, exp_name, snapshot_name + '.pth')
    ...
    torch.save(
        latest_state,os.path.join(exp_path, exp_name, 'latest_state.pth')
    )
