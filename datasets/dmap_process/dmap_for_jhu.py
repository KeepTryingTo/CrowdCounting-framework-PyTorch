...
def main_jhu(root_dir, save_dir):
    os.makedirs(save_dir, exist_ok=True)

    for phrase in ['train','test','val']:
        im_list = glob.glob(os.path.join(root_dir, phrase, 'images','*.jpg'))
        sub_save_dir = os.path.join(save_dir, phrase.lower(), 'npys')
        ...
        for img_path in tqdm(im_list):
            ...
            if (imgName.replace('.jpg', '.npy') in
                    os.listdir(os.path.join(save_dir, phrase.lower(), 'npys'))):
            ...
            save_path = os.path.join(save_dir, phrase.lower(), 'npys',
...
if __name__ == "__main__":
    ...
    save_dir = r'/home/ff/myProject/KGT/myProjects/myDataset/jhu_crowd_v2.0'
    main_jhu(root_dir, save_dir)
