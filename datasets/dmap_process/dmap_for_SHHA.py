...
def main_shaa(root_dir, save_dir):
    os.makedirs(save_dir, exist_ok=True)

    for mod in ['train', 'test']:
        sub_save_dir = os.path.join(save_dir, mod, 'npys')
        os.makedirs(sub_save_dir, exist_ok=True)
        img_root = os.path.join(root_dir, mod + '_data')
        image_file_list = os.listdir(os.path.join(img_root, 'images'))
        for image_file in tqdm(image_file_list):
            image_path = os.path.join(img_root, 'images',image_file)
            mat_path = image_path.replace('images','mats').replace('IMG','GT_IMG').replace('.jpg','.mat')
            image = plt.imread(image_path)
            mat = loadmat(mat_path)
            points = mat['image_info'][0][0][0][0][0]
            # generate densitymap
            densitymap = gaussian_filter_density_fixed(image,points)
            if (image_file.replace('.jpg','.npy') in os.listdir(sub_save_dir)):
                print(f"{image_file.replace('.jpg','.npy')} has been exist!\n")
                continue
            np.save(os.path.join(sub_save_dir,image_file.replace('.jpg','.npy')),densitymap)
            print(f'{image_file} is finished!')



if __name__ == "__main__":
    root_dir = r'/home/ff/myProject/KGT/myProjects/myDataset/shanghai/ShanghaiTech/part_A/test_data'
    save_dir = r'/home/ff/myProject/KGT/myProjects/myDataset/shanghai/ShanghaiTech/part_A/test_data/npys'

    main_shaa(root_dir, save_dir)
