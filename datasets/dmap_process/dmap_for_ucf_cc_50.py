...
def main_ucf_cc_50(root_dir, save_dir):
    os.makedirs(save_dir, exist_ok=True)

    im_list = glob.glob(os.path.join(root_dir, 'images', '*jpg'))
    mat_list = glob.glob(os.path.join(root_dir, 'mats', '*mat'))
    for img_path in tqdm(im_list):
        mat_path = img_path.split('.')[0] + '_ann.mat'
        image = plt.imread(img_path)
        mat = loadmat(mat_path.replace('images', 'mats'))
        points = mat['annPoints'].astype(np.float32)
        # TODO generate densitymap
        imgName = img_path.split('/')[-1]

        if (imgName.replace('.jpg', '.npy') in
                os.listdir(os.path.join(save_dir , 'npys'))):
            print(f"{imgName.replace('.jpg', '.npy')} has been exist!\n")
            continue
        densitymap = gaussian_filter_density_fixed(image, points)
        np.save(os.path.join(save_dir, 'npys',
                             imgName.replace('.jpg', '.npy')), densitymap)
        # TODO 保存图像
        # save_img = cv2.imread(img_path)
        # s_p = os.path.join(save_dir, 'imgs', imgName)
...
if __name__ == "__main__":
    ...
    save_dir = r'/home/ff/myProject/KGT/myProjects/myDataset/UCF_CC_50'

    main_ucf_cc_50(root_dir, save_dir)
